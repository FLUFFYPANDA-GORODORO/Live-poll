...
async function ensureConnected() {
  ...
  connection.onreconnected(() => {
    ...
    groups.forEach((pollId) => {
      const entry = listeners[pollId]?.[0];
      const sessionId = entry?.sessionId || null;
      connection.invoke("JoinPollGroup", pollId, sessionId).catch(() => {});
    });
  });
 ...
 * Subscribe to events for a specific poll.
 * @param {string} pollId
 * @param {object} handlers - { onPollUpdated, onVoteCountsUpdated, onPollEnded, onWordCloudUpdated, onReceiveBubbleData, onParticipantSubmittedCountUpdate, onRankingUpdated, onResponseAdded, onResponseDeleted }
 * @param {string} sessionId
 * @returns {function} unsubscribe function
 */
export function subscribeToPoll(pollId, handlers, sessionId) {
  ...
  const onQuestionActivated = (data) => handlers.onQuestionActivated?.(data);
  const onRankingUpdated = (data) => handlers.onRankingUpdated?.(data);
  const onResponseAdded = (data) => handlers.onResponseAdded?.(data);
  const onResponseDeleted = (data) => handlers.onResponseDeleted?.(data);
  ...
  ensureConnected()
    .then((conn) => {
      ...
      conn.on("QuestionActivated", onQuestionActivated);
      conn.on("RankingUpdated", onRankingUpdated);
      conn.on("ResponseAdded", onResponseAdded);
      conn.on("ResponseDeleted", onResponseDeleted);

      return conn.invoke("JoinPollGroup", pollId, sessionId || null);
    })
    .catch((err) => {
  ...
  const entry = { 
    ...
    onReceiveBubbleData, onParticipantSubmittedCountUpdate,
    onQuestionActivated, onRankingUpdated, onResponseAdded, onResponseDeleted,
    sessionId
  };
  ...
  return () => {
    ...
    ensureConnected()
      .then((conn) => {
        ...
        conn.off("QuestionActivated", onQuestionActivated);
        conn.off("RankingUpdated", onRankingUpdated);
        conn.off("ResponseAdded", onResponseAdded);
        conn.off("ResponseDeleted", onResponseDeleted);
