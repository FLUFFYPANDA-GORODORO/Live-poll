...
/**
 * Send ephemeral bid update.
 */
export async function sendBidChange(pollId, questionIndex, sessionId, biddingSkillId, amount) {
  const conn = await ensureConnected();
  await conn.invoke("NotifyBidChange", pollId, questionIndex, sessionId, parseInt(biddingSkillId), amount);
}
