...
export default function MasterclassPresent({
  ...
  return (
    <div className="h-screen max-h-screen bg-[url('/MCbackground.jpg')] bg-cover bg-center bg-no-repeat flex flex-col text-emerald-50 font-sans overflow-y-auto relative">
      ...
      <header className="w-full z-20 relative bg-transparent">
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-4 min-w-0 flex-1">
            ...
            <img src="/mc01.png" alt="Masterclass Logo" className="h-14 w-auto object-contain flex-shrink-0 translate-y-4" />
          </div>

          <div className="flex items-center gap-3 justify-end flex-1 flex-shrink-0">
            <div className="flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-full text-slate-300 text-sm border border-white/5">
              <Users className="w-4 h-4 text-emerald-400" />
              <span className="font-medium">{totalVotes}</span>
            </div>
            <button
              onClick={() => setShowQR(!showQR)}
              className="px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-emerald-250 text-sm border border-white/5 transition-colors"
            >
              {showQR ? "Hide" : "Show"} QR
            </button>
            <button
              onClick={toggleFullscreen}
              className="p-2 rounded-lg bg-white/10 hover:bg-white/20 text-slate-300 hover:text-white transition-colors border border-white/5"
            >
      ...
      <main className="flex-1 flex flex-col justify-between px-6 md:px-12 pt-4 pb-20 z-10 relative max-w-7xl w-full mx-auto bg-black/20 backdrop-blur-[1px] rounded-3xl border border-white/5 shadow-2xl my-4">
        ...
        {showQR && (
          <div className="absolute top-4 right-12 bg-black/80 backdrop-blur-md p-4 rounded-2xl shadow-2xl border border-white/10 flex flex-col items-center z-10">
            ...
            <p className="text-[10px] text-center mt-2 text-emerald-350 font-mono font-bold tracking-wider">{pollId}</p>
          </div>
        )}
        ...
        <div className="text-center w-full max-w-4xl mx-auto mb-3 mt-1">
          ...
          <div className="mt-3 text-slate-300 font-medium text-sm flex items-center justify-center gap-3">
            <span>Question {currentQuestionIndex + 1} of {totalQuestions}</span>
            {isVotingActive && (
              <span className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-xs">
            ...
            return (
              <div key={idx} className="flex flex-col items-center flex-1 max-w-[250px] h-[36vh] justify-end">
                ...
                <div className="w-full relative flex-1 flex items-end">
                  <div
                    className="w-full rounded-t-md transition-all duration-700 ease-out border-t border-emerald-400/20"
                    style={{
                      height: `${height || 3}%`,
                      background: gradient,
                      boxShadow: votes > 0 ? `0 4px 25px #10b98150` : "none",
                    }}
                  />
                </div>
                ...
                <div className="text-center mt-2 w-full h-20 flex items-start justify-center">
                  <div className="text-slate-150 font-bold text-sm md:text-base lg:text-lg whitespace-normal break-words w-full drop-shadow-sm px-1" title={option.text}>
                    {option.text}
                  </div>
                </div>
              </div>
      ...
      <div className="fixed bottom-2 left-1/2 -translate-x-1/2 z-20 bg-black/60 backdrop-blur-md border border-white/10 rounded-xl p-1.5 flex items-center gap-2 shadow-2xl">
        ...
        {isVotingActive ? (
          <button
            ...
            className="flex items-center gap-1 px-3.5 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-bold transition-all text-xs"
          >
