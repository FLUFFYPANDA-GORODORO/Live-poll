...
function VerticalBarChart({ options, theme }) {
  // Generate sample/mock votes just for a realistic preview in the editor
...
function QuestionSlide({ question, index, isActive, onClick, onDelete, canDelete, theme }) {
  ...
  if (isMasterclass) {
    ...
    subTextClass = "text-xs text-emerald-555";
  ...
  return (
    <div onClick={onClick} className={slideClass}>
      ...
      {/* Mini preview bar chart in sidebar slide */}
      {question.type !== "WordCloud" && question.options ? (
        ...
        <div className={`flex items-center justify-center gap-1.5 h-6 mt-3 text-[10px] font-bold rounded py-0.5 ${isActive ? "opacity-100" : "opacity-60"} ${
          isMasterclass 
            ? (isActive ? "text-white bg-white/20" : "text-emerald-700 bg-emerald-50") 
            : isSynergy 
...
export default function StandardEdit({
  ...
  let titleButtonClass = "text-xl font-bold text-text hover:text-primary";
  ...
  if (isMasterclass) {
    ...
    headerLeftBgClass = "w-96 h-full border-r border-emerald-100 flex items-center px-4 gap-3 bg-emerald-50/20";
    ...
    titleButtonClass = "text-xl font-bold text-white hover:text-rose-400 flex items-center gap-1.5";
    ...
    titleButtonClass = "text-xl font-bold text-white hover:text-sky-400 flex items-center gap-1.5";
  ...
  return (
    <div className={mainWrapperClass}>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2 rounded-lg text-white font-semibold hover:opacity-90 transition-all disabled:opacity-50 text-sm ${
                  isSynergy ? "bg-gradient-to-r from-red-500 to-rose-600" : "bg-gradient-to-r from-[#6366F1] to-[#8B5CF6]"
                }`}
              >
                {isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                Save Changes
      ...
      <div className="flex flex-1 overflow-hidden z-10">
        ...
        {/* 3. Right Sidebar Settings Panel (For all themes in 3-column layout) */}
            ...
            {activeQuestion?.type !== "WordCloud" ? (
              ...
              <div className={`p-4 border rounded-xl text-center text-xs font-semibold ${
                isMasterclass 
                  ...
                  : "bg-indigo-50/50 border-slate-200 text-[#6366F1]"
              }`}>
