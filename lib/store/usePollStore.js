...
import { subscribeToPoll, subscribeToPresenter, sendEmoji, notifySelectionChange } from "@/lib/pollHub";
...
export const usePollStore = create((set, get) => ({
  ...
  fetchBiddingPollById: async (pollId) => {
    ...
    try {
      ...
      set({ currentBiddingPoll: data, loadingBiddingCurrent: false });
      return data;
    } catch (err) {
  ...
  createBiddingPoll: async (title, theme = "default", skillCost = 20, skillIds = []) => {
    set({ isSaving: true });
    try {
      const data = await api.createBiddingPoll({
        title: title.trim(),
        theme,
        skillCost,
        skillIds,
  ...
  saveBiddingPoll: async (pollId, title, theme = "default", skillCost = 20, skillIds = []) => {
    set({ isSaving: true });
    try {
      const data = await api.updateBiddingPoll(pollId, {
        title: title.trim(),
        theme,
        skillCost,
        skillIds,
