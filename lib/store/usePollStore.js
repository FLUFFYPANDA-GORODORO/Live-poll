...
export const usePollStore = create((set, get) => ({
  ...
  nextQuestion: async (pollId, activeQuestionIndex, totalQuestions) => {
    ...
    try {
      ...
      // Stop current voting first
      await api.stopVoting(pollId);
      // Navigate to next question
      await api.nextQuestion(pollId);
      // Auto-start voting on the new question
  ...
  simulateWordCloud: (pollId, questionIndex) => {
    ...
    // Top 3 words get a large percentage of the votes (highly repeated)
    ...
    // Next 5 words get moderate votes
    ...
    // Distribute the remaining votes among the rest of the words (mostly 1s and 2s)
    let wordIdx = 8;
    while (remaining > 0 && wordIdx < buzzwords.length) {
      const count = Math.min(remaining, Math.floor(Math.random() * 2) + 1); // 1 or 2
