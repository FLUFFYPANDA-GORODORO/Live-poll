...
export const usePollStore = create((set, get) => ({
  ...
  createPoll: async (title, questions, themeId = "11111111-1111-1111-1111-111111111111", enableAudio = false, audioUrl = null) => {
    set({ isSaving: true });
    try {
      const pollHasAudio = enableAudio || questions.some((q) => q.enableAudio && q.audioUrl);
      const pollAudioUrl = audioUrl || questions.find((q) => q.audioUrl)?.audioUrl || null;

      const data = await api.createPoll({
        title: title.trim(),
        themeId: typeof themeId === "string" ? themeId : "11111111-1111-1111-1111-111111111111",
        enableAudio: Boolean(pollHasAudio),
        audioUrl: pollAudioUrl,
  ...
  savePoll: async (pollId, title, questions, themeId = "11111111-1111-1111-1111-111111111111", enableAudio = false, audioUrl = null) => {
    set({ isSaving: true });
    try {
      const pollHasAudio = enableAudio || questions.some((q) => q.enableAudio && q.audioUrl);
      const pollAudioUrl = audioUrl || questions.find((q) => q.audioUrl)?.audioUrl || null;
